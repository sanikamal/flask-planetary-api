...
app.config['JWT_SECRET_KEY'] = 'easy-secret'
...
def db_seed():
    ...
    test_user = User(first_name="Sani",
                     last_name="Kamal",
                     email="test@test.com",
                     password="password")
...
def retrieve_password(email: str):
    ...
    if user:
        msg = Message("Your Rua Planetary API Password id:" + user.password,
                      sender="admin@ruaplanetary-api.in", recipients=[email])
        mail.send(msg)
        return jsonify(message="Password send to " + email)
    else:
        return jsonify(message="This email doe's not exist."), 401
...
def planet_details(planet_id: int):
    ...
    if planet:
        ...
        return jsonify(message="That planet doe's not exist."), 404
...
def add_planet():
    ...
    if test:
        ...
        db.session.commit()
        return jsonify(message="Added new Planet"), 201
